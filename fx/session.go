...
func NewSessionStore(cfg SessionConfig, lc fx.Lifecycle, db *sql.DB) scs.Store {
	store := postgresstore.NewWithCleanupInterval(db, cfg.CleanupInterval)

	lc.Append(fx.StopHook(store.StopCleanup))
